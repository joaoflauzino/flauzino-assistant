...
class CategoryService:
    ...
    @handle_service_errors
    async def list(self, page: int = 1, size: int = 100) -> tuple[list[CategoryResponse], int]:
        skip = (page - 1) * size
    ...
    @handle_service_errors
    async def update(self, category_id: UUID, update_data: CategoryUpdate) -> CategoryResponse:
