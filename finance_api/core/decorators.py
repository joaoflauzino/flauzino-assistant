...
def handle_categories_errors(func: Callable[..., Any]) -> Callable[..., Any]:
    """Decorator to catch unexpected errors in Category Service."""
    ...
    async def wrapper(*args: Any, **kwargs: Any) -> Any:
        try:
            ...
            if isinstance(e, HTTPException):
                raise
            logger.error(f"Category service error: {e}", exc_info=True)
            raise CategoryServiceError(f"Internal Server Error: {str(e)}")
