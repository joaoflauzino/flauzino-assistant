...
from finance_api.core.decorators import handle_payment_method_errors
...
class PaymentMethodService:
    ...
    async def create(self, method_data: PaymentMethodCreate) -> PaymentMethod:
        ...
        if existing:
            raise EntityConflictError(f"Payment method with key '{method_data.key}' already exists.")
    ...
    async def update(
        ...
        if update_data.key:
            ...
            if existing and existing.id != method_id:
                raise EntityConflictError(f"Payment method with key '{update_data.key}' already exists.")
